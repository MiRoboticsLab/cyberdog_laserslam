...
ConstraintBuilder::ConstraintBuilder(
    ...
    : param_(param), thread_pool_(thread_pool), when_done_(nullptr),
      reloc_when_done_(nullptr),
      finish_node_task_(absl::make_unique<common::Task>()),
      when_done_task_(absl::make_unique<common::Task>()),
      finish_reloc_node_task_(absl::make_unique<common::Task>()),
      reloc_when_done_task_(absl::make_unique<common::Task>()),
...
void ConstraintBuilder::FindRelocLoopConstraint(
    ...
    if (reloc_when_done_) {
        LOG(WARNING) << "Reloc Loop Closure was called while RelocWhenDone was "
                        "scheduled";
    }
    // while (reloc_when_done_) {
    //     usleep(1000);
    //     // LOG(WARNING) << "Find Reloc Loop was called while WhenDone was
    //     // schedule";
    // }
    reloc_constraints_.emplace_back();
    auto *const constraint = &reloc_constraints_.back();
    const auto *scan_matcher =
        DispatchScanMatcherConstrution(submap_id, submap->grid());
    auto constraint_task = absl::make_unique<common::Task>();
    constraint_task->SetWorkItem([=]() LOCKS_EXCLUDED(reloc_mutex_) {
        ComputeConstraint(submap_id, submap, node_id, constant_data,
                          initial_relative_pose, *scan_matcher, true,
                          constraint);
    });
    constraint_task->AddDependency(scan_matcher->creation_task_handle);
    auto constraint_task_handle =
        thread_pool_->Schedule(std::move(constraint_task));
    finish_reloc_node_task_->AddDependency(constraint_task_handle);
}
...
void ConstraintBuilder::AddLocalLoopConstraint(
    ...
    constraint_task->SetWorkItem([=]() LOCKS_EXCLUDED(mutex_) {
        ComputeConstraint(submap_id, submap, node_id, constant_data,
                          initial_relative_pose, *scan_matcher, false,
                          constraint);
    });
    constraint_task->AddDependency(scan_matcher->creation_task_handle);
    auto constraint_task_handle =
...
void ConstraintBuilder::ComputeConstraint(
    ...
    ceres_scan_matcher_.Match(
        pose_estimated.translation(), pose_estimated,
        constant_data->filtered_gravity_aligned_point_cloud,
        *submap_scan_matcher.grid, &pose_estimated, &summary);
    ...
    if (param_.log_matches) {
        ...
        const transform::Rigid2d difference =
            init_pose.inverse() * pose_estimated;
...
ConstraintBuilder::DispatchScanMatcherConstrution(
    ...
    scan_matcher_task->SetWorkItem([&submap_scan_matcher, &param]() {
        submap_scan_matcher.fast_correlative_scan_matcher = absl::make_unique<
            mapping::scan_matching::FastCorrelativeScanMatcher2D>(
...
void ConstraintBuilder::NotifyEndOfOneRelocNode() {
    ...
    CHECK(finish_reloc_node_task_ != nullptr);
    finish_reloc_node_task_->SetWorkItem([this] {
        absl::MutexLock locker(&reloc_mutex_);
        ++num_reloc_nodes_;
        LOG(INFO) << "num reloc nodes is: " << num_reloc_nodes_;
    });
    auto finish_node_task_handle =
        thread_pool_->Schedule(std::move(finish_reloc_node_task_));
    finish_reloc_node_task_ = absl::make_unique<common::Task>();
    reloc_when_done_task_->AddDependency(finish_node_task_handle);
    ++num_reloc_started_nodes_;
}
...
void ConstraintBuilder::RelocWhenDone(
    const std::function<void(const ConstraintBuilder::Result &)> &callback) {
    absl::MutexLock locker(&reloc_mutex_);
    CHECK(reloc_when_done_ == nullptr);
    reloc_when_done_ =
        absl::make_unique<std::function<void(const Result &)>>(callback);
    CHECK(reloc_when_done_task_ != nullptr);
    reloc_when_done_task_->SetWorkItem([this] { RunRelocWhenDoneCallback(); });
    thread_pool_->Schedule(std::move(reloc_when_done_task_));
    reloc_when_done_task_ = absl::make_unique<common::Task>();
    // Result result;
...
void ConstraintBuilder::RunRelocWhenDoneCallback() {
    Result result;
    std::unique_ptr<std::function<void(const Result &)>> callback;
    {
        ...
        if (param_.log_matches) {
            LOG(INFO) << reloc_constraints_.size()
                      << " computations resulted in " << result.size()
                      << " additional constraints.";
        }
        reloc_constraints_.clear();
        callback = std::move(reloc_when_done_);
        reloc_when_done_.reset();
    }
    (*callback)(result);
}
...
void ConstraintBuilder::RunWhenDoneCallback() {
    ...
    (*callback)(result);
    // when the scan matching task time consuming, notify function was not
    // execute yet TO_DO(feixiang) : change the strategy, judge is finished
    // until all task end
}
