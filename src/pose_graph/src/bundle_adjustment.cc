...
namespace {
protos::mapping::proto::PoseGraph::Constraint::Tag ToProto(
...
protos::mapping::proto::PoseGraph::Constraint ToProto(
...
NodeId BundleAdjustment::AddFirstRelocNode(
  ...
  std::shared_ptr<const pose_graph::optimization::TrajectoryNode> node =
      std::make_shared<const pose_graph::optimization::TrajectoryNode>(
          pose_graph::optimization::TrajectoryNode{constant_data,
...
bool BundleAdjustment::RecoverPoseGraphFromLast(
  ...
  for (auto node : nodes) {
    ...
    optimization_problem_->InsertTrajectoryNode(
        node.id,
        NodeSpec2D{node.data.constant_data->time,
                   transform::Project2D(node.data.constant_data->local_pose *
...
void BundleAdjustment::RunOptimization() {
  ...
  for (const int trajectory_id : node_data.trajectory_ids()) {
    ...
    // Extrapolate all point cloud poses that were not included in 'optimization
    // problem' yet
    // still have node join Added newly, but not optimized yet
...
transform::Rigid3d BundleAdjustment::GetLocalToGlobalTransform(
...
transform::Rigid3d BundleAdjustment::ComputeLocalToGlobalTransform(
  ...
  if (begin_it == end_it) {
    // we do not have initial poses from other trajectory, we also do not do the
    // connective trajectory work
...
void BundleAdjustment::RelocConstraintsWorkHandle(
    const ConstraintBuilder::Result& result) {
  if (result.size() > 0) {
    LOG(INFO) << "Found Reloc Constraints, Which Size is: " << result.size();
    ...
    if (state.state == TrajectoryState::FROZEN) {
      const transform::Rigid2d initial_relative_pose =
          optimization_problem_->submap_data()
              ...
              .global_pose.inverse() *
          transform::Project2D(data_.trajectory_nodes.at(node_id).global_pose);
      const mapping::Submap2D* submap = static_cast<const mapping::Submap2D*>(
...
SubmapData BundleAdjustment::GetSubmapDataUnderLock(
...
mapping::MapById<NodeId, TrajectoryNode> BundleAdjustment::GetTrajectoryNodes()
    const {
  absl::MutexLock locker(&pose_graph_mutex_);
...
void BundleAdjustment::TrimSubmap(const SubmapId& id) {
  ...
  std::set_difference(data_.submap_data.at(id).node_ids.begin(),
                      data_.submap_data.at(id).node_ids.end(),
...
protos::mapping::proto::PoseGraph BundleAdjustment::ToProto(
  ...
  for (const auto& submap_id_data : GetSubmapDataUnderLock()) {
    ...
    *submap_data.mutable_submap() = submap_id_data.data.submap->ToProto(true);
    *submap_data.mutable_global_pose() = global_pose;
    submap_data.mutable_id()->set_submap_index(submap_id_data.id.submap_index);
  ...
  for (const auto& node_id_data : GetTrajectoryNodes()) {
    ...
    *data.mutable_filtered_gravity_aligned_point_cloud() =
        node_id_data.data.constant_data->filtered_gravity_aligned_point_cloud
            .ToProto();
