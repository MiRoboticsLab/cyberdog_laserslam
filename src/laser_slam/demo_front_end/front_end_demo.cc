...
class FrontEndDemo : public rclcpp::Node {
  ...
  bool Initialization() {
    ...
    laser_subscription_ =
        this->create_subscription<sensor_msgs::msg::LaserScan>(
            ...
            std::bind(&FrontEndDemo::LaserCallBack, this,
                      std::placeholders::_1),
            sub_laser_opt);
 ...
 private:
  ...
  void LaserCallBack(const sensor_msgs::msg::LaserScan::SharedPtr laser) {
    ...
    std::unique_ptr<MatchingResult> tmp_result = local_slam_->AddRangeData(pc);
    if (tmp_result != nullptr) {
      if (tmp_result->insertion_result != nullptr) {
        const auto node = tmp_result->insertion_result->node_constant_data;
        const auto submaps = tmp_result->insertion_result->insertion_submaps;
      ...
      for (size_t j = 0; j < tmp_result->range_data_in_local.returns.size();
           ++j) {
