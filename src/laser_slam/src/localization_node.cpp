...
#include "laser_slam/localization_node.hpp"
using namespace std::chrono_literals;
namespace cartographer {
namespace laser_slam {
LocalizationNode::LocalizationNode()
    : nav2_util::LifecycleNode("localization", ""), reloc_id_(0),
      last_laser_time_(0), reloc_client_(nullptr), localization_(nullptr),
      reloc_thread_(nullptr), pose_recorder_(nullptr) {}

LocalizationNode::~LocalizationNode() {}

nav2_util::CallbackReturn
LocalizationNode::on_configure(const rclcpp_lifecycle::State &state) {
    ...
    this->declare_parameter(
        ...
        "distance");
    this->get_parameter("pose_graph_param.constraint_builder_param.max_reloc_"
                        "constraint_distance",
    ...
    laser_subscription_ =
        this->create_subscription<sensor_msgs::msg::LaserScan>(
            ...
            std::bind(&LocalizationNode::LaserCallBack, this,
                      std::placeholders::_1),
            sub_laser_opt);
...
nav2_util::CallbackReturn
LocalizationNode::on_activate(const rclcpp_lifecycle::State &state) {
...
nav2_util::CallbackReturn
LocalizationNode::on_deactivate(const rclcpp_lifecycle::State &state) {
    localization_->Stop();
...
nav2_util::CallbackReturn
LocalizationNode::on_cleanup(const rclcpp_lifecycle::State &state) {
    localization_.reset();
    return nav2_util::CallbackReturn::SUCCESS;
}

nav2_util::CallbackReturn
LocalizationNode::on_shutdown(const rclcpp_lifecycle::State &state) {
    LOG(INFO) << "Shutting Down";
    return nav2_util::CallbackReturn::SUCCESS;
}

void LocalizationNode::PublishPoseThread() {
    if (is_on_active_status_ && localization_ != nullptr) {
        auto tt = tf2::get_now();
        auto msg_time = tf2_ros::toMsg(tt);

        common::Time time_l = common::FromUniversal(
            (msg_time.sec + common::kUtsEpochOffsetFromUnixEpochInSeconds) *
                10000000ll +
            (msg_time.nanosec + 50) / 100);
        transform::Rigid3d pose = localization_->Extrapolate(time_l);
...
void LocalizationNode::PosePcCallBack(const transform::Rigid3d &pose,
    ...
    common::Time time_l = common::FromUniversal(
        ...
        (msg_time.nanosec + 50) / 100);
    LOG(INFO) << time_l;
    auto pp = localization_->Extrapolate(time_l);
    LOG(INFO) << "msg_time: " << msg_time.sec;
    nav_msgs::msg::Odometry odometry;
    odometry.header.frame_id = "laser_odom";

    odometry.header.stamp = msg_time;
    odometry.child_frame_id = "base_footprint";
    odometry.pose.pose.position.x = pp.translation().x();
    odometry.pose.pose.position.y = pp.translation().y();
    odometry.pose.pose.position.z = pp.translation().z();
    odometry.pose.pose.orientation.x = pp.rotation().x();
    odometry.pose.pose.orientation.y = pp.rotation().y();
    odometry.pose.pose.orientation.z = pp.rotation().z();
    odometry.pose.pose.orientation.w = pp.rotation().w();
...
void LocalizationNode::StartLocationCallback(
    ...
    tf_publisher_ = create_wall_timer(
        65ms, std::bind(&LocalizationNode::PublishPoseThread, this));
