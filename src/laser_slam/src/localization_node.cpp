...
LocalizationNode::on_configure(const rclcpp_lifecycle::State & state)
{
  ...
  this->declare_parameter(
    "pose_graph_param.constraint_builder_param.loop_"
  ...
  this->declare_parameter(
    ...
    "distance");
  this->get_parameter(
    "pose_graph_param.constraint_builder_param.max_reloc_"
    "constraint_distance",
  ...
  this->declare_parameter(
    "pose_graph_param.constraint_builder_param.ceres_"
  ...
  start_location_service_ = create_service<std_srvs::srv::SetBool>(
    start_location_service_name,
    std::bind(
      &LocalizationNode::StartLocationCallback, this,
  ...
  stop_location_service_ = create_service<std_srvs::srv::SetBool>(
    stop_location_service_name,
    std::bind(
      &LocalizationNode::StopLocationCallback, this,
      std::placeholders::_1, std::placeholders::_2));
...
void LocalizationNode::PosePcCallBack(
  ...
  const sensor::RangeData & pc)
{
  ...
  if (pose_recorder_ != nullptr) {
    pose_recorder_->InputRecordPose(
      common::ToUniversal(pc.returns.time()),
