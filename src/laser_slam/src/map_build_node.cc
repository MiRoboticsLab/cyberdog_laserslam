...
nav2_util::CallbackReturn MapBuildNode::on_configure(
  ...
  // Final Map saver
  grid_.reset(
      new GridForNavigation(0.05, param.submap_param.probability_insert_param));
  ...
  this->get_parameter("start_mapping_service_name", start_mapping_service_name);
...
nav2_util::CallbackReturn MapBuildNode::on_activate(
    const rclcpp_lifecycle::State& state) {
...
nav2_util::CallbackReturn MapBuildNode::on_deactivate(
    const rclcpp_lifecycle::State& state) {
...
nav2_util::CallbackReturn MapBuildNode::on_cleanup(
    const rclcpp_lifecycle::State& state) {
  pose_graph_.reset();
...
nav2_util::CallbackReturn MapBuildNode::on_shutdown(
    const rclcpp_lifecycle::State& state) {
...
void MapBuildNode::StartMappingCallback(
  ...
  if (get_current_state().id() !=
    ...
    LOG(ERROR) << "Received Trigger Mapping request But not in active state, "
                  "ignoring!!!";
    return;
  }
...
void MapBuildNode::LaserCallBack(
  ...
  if (local_matching_result != nullptr) {
    ...
    t.transform.rotation.x = local_matching_result->local_pose.rotation().x();
    t.transform.rotation.y = local_matching_result->local_pose.rotation().y();
    t.transform.rotation.z = local_matching_result->local_pose.rotation().z();
    t.transform.rotation.w = local_matching_result->local_pose.rotation().w();
    ...
    for (size_t j = 0;
         j < local_matching_result->range_data_in_local.returns.size(); ++j) {
      pcl_cloud.points[j].x =
          local_matching_result->range_data_in_local.returns[j].position.x();
      pcl_cloud.points[j].y =
          local_matching_result->range_data_in_local.returns[j].position.y();
      pcl_cloud.points[j].z =
          local_matching_result->range_data_in_local.returns[j].position.z();
    }
