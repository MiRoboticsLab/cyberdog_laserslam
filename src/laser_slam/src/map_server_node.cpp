...
void MapServerNode::AddRangeDataAndPose(
  ...
  while (!quit_thread_) {
    ...
    if (insertion_map_ != nullptr) {
      if (num_accumulated_range_data_ > 20) {
        ...
        insertion_map_bk->grid()->ComputeCroppedLimits(
          &offset,
          &cell_limits);
        ...
        if (publish_grid_ != nullptr) {
          ...
          int width_offset_v =
            std::ceil(
            (grid_last.info.origin.position.x -
            pub_grid.info.origin.position.x) /
            0.05);
          int height_offset_v =
            std::ceil(
            (grid_last.info.origin.position.y -
...
void MapServerNode::MapTaskThread()
{
  while (!quit_thread_) {
    ...
    if (insertion_map_ != nullptr) {
      ...
      std::lock_guard<std::mutex> lk(insertion_map_mtx_);
      insertion_map_->InsertRangeData(
        range_data,
