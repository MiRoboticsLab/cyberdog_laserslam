...
PrecomputationGridStack2D::PrecomputationGridStack2D(
    const Grid2D& grid, const FastCorrelativeScanMatcherParam& param) {
  CHECK_GE(param.branch_and_bound_depth, 1);
  const int max_width = 1 << (param.branch_and_bound_depth - 1);
  precomputation_grids_.reserve(param.branch_and_bound_depth);
  std::vector<float> reusable_intermediate_grid;
  const CellLimits limits = grid.limits().cell_limits();
  reusable_intermediate_grid.reserve((limits.num_x_cells + max_width - 1) *
                                     limits.num_y_cells);
  for (int i = 0; i != param.branch_and_bound_depth; ++i) {
    const int width = 1 << i;
...
FastCorrelativeScanMatcher2D::FastCorrelativeScanMatcher2D(
    ...
    : param_(param),
      ...
      precomputation_grid_stack_(
          absl::make_unique<PrecomputationGridStack2D>(grid, param)) {}
...
bool FastCorrelativeScanMatcher2D::Match(
  ...
  const SearchParameters search_parameters(param_.linear_search_window,
                                           param_.angular_search_window,
