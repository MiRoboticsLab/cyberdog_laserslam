...
double RealTimeCorrelativeScanMatcher2D::Match(
  ...
  const SearchParameters search_parameters(
      param_.linear_search_window, param_.angular_search_window,
      rotated_point_cloud, grid.limits().resolution());
  const std::vector<sensor::PointCloud> rotated_scans =
      GenerateRotatedScans(rotated_point_cloud, search_parameters);
  std::chrono::steady_clock::time_point time1 =
      std::chrono::steady_clock::now();
  std::chrono::duration<double> duration = time1 - start_time;
  ...
  for (Candidate2D& candidate : *candidates) {
    ...
    candidate.score *= std::exp(-common::Pow2(
        std::hypot(candidate.x, candidate.y) *
            param_.translation_delta_cost_weight +
        std::abs(candidate.orientation) * param_.rotation_delta_cost_weight));
  }
}
