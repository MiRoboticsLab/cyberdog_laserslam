...
#ifndef RANGE_DATA_MATCHING_MAP_SUBMAP_2D_H_
#define RANGE_DATA_MATCHING_MAP_SUBMAP_2D_H_
...
constexpr int kProbabilityGridType = 0;
...
class Submap2D : public Submap {
 public:
  ...
  explicit Submap2D(const protos::mapping::proto::Submap2D& proto,
...
class ActiveSubmaps2D {
 public:
  ActiveSubmaps2D(const SubMapParam& param)
      : param_(param), range_data_inserter_(CreateRangeDataInserter()) {}
 ...
 private:
  std::unique_ptr<RangeDataInserterInterface> CreateRangeDataInserter() {
    if (param_.grid_insert_type == kProbabilityGridType) {
      return absl::make_unique<ProbabilityGridRangeDataInserter2D>(
          param_.probability_insert_param);
    } else {
      return nullptr;
    }
  }
...
#endif  // RANGE_DATA_MATCHING_MAP_SUBMAP_2D_H_
