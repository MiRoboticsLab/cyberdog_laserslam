...
std::unique_ptr<PoseExtrapolator> PoseExtrapolator::InitialWithImu(
...
Eigen::Quaterniond PoseExtrapolator::ExtrapolateRotation(
    common::Time time, filter::ImuTracker* const imu_tracker) {
...
void PoseExtrapolator::AddPose(const transform::TimedRigid3d& pose) {
  ...
  if (imu_tracker_ == nullptr) {
    ...
    imu_tracker_ = std::make_unique<filter::ImuTracker>(gravity_time_constant_,
                                                        tracker_start);
  }
